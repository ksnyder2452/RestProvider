...
class JenkinsControllerIntegrationTest {
    ...
    void setup() {
        ...
        registry.setControllerEnabled("Jenkins", true);
    ...
    void shouldAllowPutWhenPasscodeValid() throws Exception {
        ...
        request.addHeader("jenkinsUser", "abc");
        request.addHeader("jenkinsApiToken", "xyz");
    ...
    @Test
    void shouldSupportQueryParamsAndBaseUrl() throws Exception {
        BasicClassicHttpRequest request = new BasicClassicHttpRequest(
                "GET",
                "/api/jenkins?passCode=valid-passcode&subURI=job/test&jenkinsBaseUrl=http://jenkins.local:8080"
                        + "&jenkinsUser=abc&jenkinsApiToken=xyz");
    ...
    @Test
    void shouldReturnBadRequestWhenSubUriMissing() throws Exception {
        BasicClassicHttpRequest request = new BasicClassicHttpRequest("GET", "/api/jenkins");
        request.addHeader("passCode", "valid-passcode");
        request.addHeader("serverName", "jenkins.local");
        request.addHeader("jenkinsUser", "abc");
